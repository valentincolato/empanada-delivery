...
const COLUMNS = [
  { status: 'pending', label: 'Pending', header: 'border-amber-500 bg-amber-100 text-amber-600', badge: 'bg-amber-500' },
  { status: 'confirmed', label: 'Confirmed', header: 'border-blue-500 bg-blue-100 text-blue-600', badge: 'bg-blue-500' },
  { status: 'preparing', label: 'Preparing', header: 'border-violet-500 bg-violet-100 text-violet-600', badge: 'bg-violet-500' },
  { status: 'ready', label: 'Ready', header: 'border-emerald-500 bg-emerald-100 text-emerald-600', badge: 'bg-emerald-500' },
]
...
export default function OrdersDashboard() {
  ...
  if (loading) return <div className="flex justify-center p-16 font-sans text-slate-500">Loading orders...</div>
          ...
          return (
            <div key={status} className="overflow-hidden rounded-xl bg-white shadow" data-testid={`column-${status}`}>
              <div className={`flex items-center justify-between border-t-4 px-4 py-3 ${header}`}>
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs font-bold text-white ${badge}`}>{colOrders.length}</span>
              </div>
              <div className="flex flex-col gap-3 p-3">
                {colOrders.length === 0 && <div className="py-6 text-center text-sm text-slate-400">No orders</div>}
                {colOrders.map((order) => (
                  <OrderCard
                    key={order.id}
                    order={order}
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4" data-testid="order-card">
      <div className="mb-1 flex justify-between">
        ...
        <span className="text-xs text-slate-400">{ago}m ago</span>
      </div>
